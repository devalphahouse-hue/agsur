...
class AGSurBackOfficeSupabaseUser extends BaseAuthUser {
  AGSurBackOfficeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> aGSurBackOfficeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AGSurBackOfficeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
