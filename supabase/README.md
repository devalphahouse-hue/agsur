...
| 20260722200000 | `backfill_stuck_emails` | **Backfill dos e-mails presos** (fim do 422 legado): tombstone + ban nos usuários soft-deleted com e-mail não-liberado (51, incl. os 49 excluídos por UPDATE direto — decisão do dono: banidos) e nas contas de auth órfãs (sem `public.users`; guarda de idade 1h protege conversão em voo). Idempotente; nunca toca cadastro ativo |
| 20260722201000 | `admin_release_stuck_email` | RPC de **autocura do 422**: libera e-mail preso em conta excluída ou órfã (tombstone + ban + revoga sessões) e devolve `livre`/`ativo`/`liberado`. Conta ativa NUNCA é tocada. Chamada pelo painel na conversão proposta→contrato e na criação manual de cliente quando o signup recusa sem cadastro ativo para reusar. Perfis do funil |
| 20260722202000 | `revoke_anon_admin_rpcs` | Revoga EXECUTE de `anon` nas 5 RPCs administrativas (`admin_delete_app_user`, `admin_purge_orphan_auth_user`, `admin_update_client_email`, `admin_reset_client_password`, `admin_release_stuck_email`) — as default privileges do Supabase concediam o grant direto a anon e o `revoke from public` não o removia. Sem exposição real (gate interno 42501); defesa em camadas |
| 20260922120000 | `stock_movements` | **Estoque de aeronaves com livro-razão** (aplicada e conferida no banco em 2026-09-22). `stock_movements` (entradas/saídas imutáveis, só por função) + `available_aircraft_logs` (log de campo por trigger) + `available_aircrafts.in_stock`/`registration_prefix` + serial obrigatório e único (case-insensitive) + `aircrafts.featured` (destaque) + `proposal.available_aircraft_id`. RPCs `stock_entry`/`stock_exit`/`stock_reentry` (documentação/master). Triggers no `contract`: herda a aeronave da proposta, **saída na conversão**, estorno no cancelamento/troca. Guarda própria `tg_available_aircrafts_guard` no lugar da `hardening_require_documentacao` (aceita a sincronização interna via flag `agsur.stock_sync`, que PostgREST não expõe). Views `vw_stock_units`/`vw_stock_movements` (`security_invoker`). Backfill de saldo inicial |
| 20260922160000 | `stock_delivery_estimate` | Entrega estimada = **fabricação + 60 dias** (pedido do cliente, 2026-09-22): helper `stock_delivery_offset_days()` + `stock_entry` derivando `delivery_date` e `entry_year` quando não vêm no payload. `available_aircrafts.configuration_deadline` deixa de ser NOT NULL (a planilha do cliente só traz Modelo/Série/Fabricação) |
| 20260922180000 | `capture_rpcs_and_status_check` | **Captura de RPC não versionada + vocabulário de status** (aplicada e conferida no banco em 2026-09-22). `get_aircraft_details_by_proposal` e `fn_available_aircrafts` só existiam no banco vivo — corpos extraídos com `pg_get_functiondef` e versionados **sem mudar comportamento**, ganhando `search_path = public` e `revoke` de `anon` (as duas eram executáveis por anon por herança do default ACL; sem vazamento, porque são SECURITY INVOKER e as policies são todas `TO authenticated`). Mais o CHECK `ck_available_aircrafts_status` (Disponível/Em negociação/Reservado/Vendido/Entregue/Baixado), união do que a UI usa com o que já existia no banco — nenhuma linha existente fica ilegal |
